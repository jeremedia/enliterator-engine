module Enliterator
  ...
  # `Visit.tokens` by facet (and tier) over the immutable visit history. No
  ...
  #
  #   Enliterator::Spend.by_facet(price_map: {
  ...
  module Spend
    ...
    # Group visit token usage by facet (and tier) from the Visit ledger.
    #
    ...
    # @param facet   [String, Symbol, nil] restrict to a single facet.
    ...
    #   to each facet. Default nil → tokens only (no dollar guesses).
    #
    # @return [Hash{String => Hash}] `{facet => {tokens:, by_tier:, [cost_usd:]}}`.
    def by_facet(host: nil, since: nil, facet: nil, price_map: nil)
      scope = visit_scope(since: since, facet: facet)

      result = Hash.new { |h, s| h[s] = { tokens: zero_tokens, by_tier: {} } }

      scope.find_each do |visit|
        bucket = result[visit.facet.to_s]
